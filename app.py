# CÓDIGO COMPLETO app.py FINAL v3
import os
import re
import pandas as pd
import pdfplumber
...
def processar_pdf(caminho_pdf, apenas_validar=False):
    try:
        with pdfplumber.open(caminho_pdf) as pdf:
            ...
            if not codigo_fornecedor: return None, None
            if apenas_validar: return codigo_fornecedor, None
            dados_completos = []
            for page in pdf.pages:
                tabelas = page.extract_tables()
                for tabela in tabelas:
                    if tabela:
                        if "Código Fornecedor" in str(tabela[0]): dados_completos.extend(tabela[1:])
                        else: dados_completos.extend(tabela)
...
def validar_pdf():
    if 'pdf_file' not in request.files or not request.files['pdf_file'].filename: return jsonify({'success': False, 'message': 'Nenhum arquivo enviado.'})
    file = request.files['pdf_file']
    if not file.filename.lower().endswith('.pdf'): return jsonify({'success': False, 'message': 'Arquivo inválido. Apenas PDFs.'})
...
def enviar_para_edicao():
    ...
    if not os.path.exists(filepath): flash("Erro: PDF não encontrado. Por favor, envie novamente."); return redirect(url_for('tela_x'))
    ...
    novo_chamado = Chamado(nome_solicitante=request.form['nome_solicitante'], email=request.form['email'], razao_social=request.form['razao_social'], codigo_fornecedor_pdf=codigo, dados=df_produtos.to_dict('records'), status='Aguardando Edição')
...
def login():
    if request.method == 'POST':
        if request.form['password'] == ADMIN_PASSWORD: session['logged_in'] = True; flash('Login bem-sucedido!'); return redirect(url_for('tela_y'))
...
def salvar_chamado(chamado_id):
    chamado = Chamado.query.get_or_404(chamado_id); dados_processados = []
    ...
    for i in range(total_linhas):
        ...
        if linha['Código Fornecedor'].strip() and linha['Descrição dos Produtos'].strip(): dados_processados.append(linha)
    chamado.dados = dados_processados; chamado.status = 'Pendente'; db.session.commit()
    return redirect(url_for('sucesso'))

@app.route('/download/<int:chamado_id>')
@login_required
def download_excel(chamado_id):
    chamado = Chamado.query.get_or_404(chamado_id); excel_file = gerar_excel(chamado.dados)
    nome_arquivo = f"relatorio_{chamado.razao_social.replace(' ','_')}_{chamado.id}.xlsx"
...
def concluir_chamado(chamado_id):
    chamado = Chamado.query.get_or_404(chamado_id); chamado.status = 'Concluído'; chamado.hora_conclusao = datetime.utcnow(); db.session.commit()
    flash(f'Chamado #{chamado_id} concluído.'); return redirect(url_for('tela_y'))

@app.route('/deletar/<int:chamado_id>')
@login_required
def deletar_chamado(chamado_id):
    chamado = Chamado.query.get_or_404(chamado_id); db.session.delete(chamado); db.session.commit()
    flash(f'Chamado #{chamado_id} deletado.'); return redirect(url_for('tela_y'))
